...
HISTORICAL_RATES: Dict[str, List[float]] = {
    # year 2013-2025 Fed Funds approximate averages
...
def run_backtest(
    ...
    # Simulate CMSI differential as mean-reverting process with signal threshold
    cmsi_series = _simulate_cmsi_signal(n, threshold)

    # Run strategy
    equity    = capital
    equity_curve: list[dict] = [{"t": dates[0].isoformat(), "v": round(equity, 2)}]
    hodl_curve:   list[dict] = [{"t": dates[0].isoformat(), "v": round(equity, 2)}]
    trades: list[dict] = []
    ...
    entry_equity   = 0.0
    trade_cmsi     = 0.0
    rebal_counter  = 0        # rebalance every ~22 trading days
    ...
    for i in range(1, n):
        ...
        if rebal_counter >= 22 or i == n - 1:
            ...
            if not in_trade:
                ...
                # Exit on signal flip or max hold
    ...
    std_ret      = math.sqrt(sum((r - avg_ret)**2 for r in returns) / len(returns)) if returns else 1e-10
    ...
    return {
        ...
        "trades":         trades[-100:],  # last 100
    }


def _simulate_cmsi_signal(n: int, threshold: float) -> list[float]:
    """Mean-reverting CMSI differential signal process (Ornstein-Uhlenbeck)."""
    random.seed(123)
    mean_rev = 0.95   # strong mean reversion
    vol      = 1.2
    s        = [0.0]
    for _ in range(n - 1):
        prev = s[-1]
        ds   = -mean_rev * 0.05 * prev + vol * random.gauss(0, 1) * 0.1
        # Inject regime: periodic strong trends
        if len(s) % 60 == 0:
            ds += random.choice([-1, 1]) * threshold * 0.8
        s.append(prev + ds)
    return s
