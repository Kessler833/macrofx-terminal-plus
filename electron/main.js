...
function waitForBackend(maxMs = 25000) {
  return new Promise((resolve, reject) => {
    ...
    function retry() {
      if (Date.now() - start > maxMs) return reject(new Error('Backend timeout'))
      setTimeout(poll, 600)
    }
    poll()
  })
      ...
      callback({
        responseHeaders: {
          ...
          'Content-Security-Policy': [
            "default-src 'self' 'unsafe-inline' 'unsafe-eval' http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* https://*"
          ]
        }
      })
    })
